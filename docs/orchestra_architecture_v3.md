...
### 3.2. Логические компоненты и контейнеры

- **orchestra-web** – SPA (React + TypeScript + Tailwind + Shadcn/ui), статика через nginx.
- **orchestra-api** – REST API, доменная логика и централизованный **Scheduler**. Компонент отвечает за управление артефактами, пользователями и планирование запусков, а также содержит **`Data Resolver` / `Agent Toolbox`** и сторожевой процесс **`Reaper`**.
- **orchestra-executor** – **Stateless Worker**. Подхватывает задачи из очереди после решения Scheduler и выполняет шаги сценариев.
- **ai-service** – **Java AI Core**. Сервис на Spring Boot + Spring AI. Содержит:
  - **Java Agent Runtime:** движок выполнения агентов.
  - **Prompt Engine:** модуль управления промптами (БД + Кэш).
  - **Tool Registry:** реестр Java-инструментов.
  - Взаимодействует с **Ollama** (модель `gpt-oss:20b`).
- **postgres** – Основная БД Orchestra c расширениями **PGVector** и **Row-Level Security (RLS)**.
- **mq** – Очередь задач (RabbitMQ).
- **keycloak** – SSO.
- **observability stack** – Prometheus, Grafana, Loki, Jaeger.
- **plantuml-server** – Рендерит Sequence-диаграммы (PUML → SVG) по HTTP для UI.

Контейнеры `orchestra-api`, `orchestra-executor` и `ai-service` горизонтально масштабируются.

---

### 3.5. Структура кода и модулей

Помимо контейнеров, серверная часть организована как Maven-мономоно репозиторий с несколькими модулями.

- **`orchestra-domain` (библиотека):**
  - Не является запускаемым сервисом.
  - Содержит JPA-сущности, Spring Data репозитории, общие DTO и мапперы.
  - Выступает единым источником правды для модели данных и подключается как зависимость в остальных сервисах.
- **`orchestra-api` (приложение):**
  - Spring Boot сервис, предоставляет REST API, Scheduler, интеграцию с `ai-service`.
  - Зависит от `orchestra-domain`.
- **`orchestra-executor` (приложение):**
  - Spring Boot worker, выполняющий сценарии, полученные из очереди.
  - Зависит от `orchestra-domain`.
- **`ai-service` (приложение):**
  - Spring Boot сервис со Spring AI и Ollama.
  - Не зависит от `orchestra-domain`, содержит собственную модель данных.

Благодаря общей библиотеке `orchestra-domain` изменения в схеме БД, DTO или репозиториях выполняются один раз и становятся доступными всем сервисам.
...
- processId: UUID
- version: int
- sourceType: enum { BPMN, PLANTUML } // Добавлен PLANTUML
- sourceUri: string
...
`controlFlowGraph` описывает узлы, gateway-типы, параллельность и циклы (с ограничениями maxLoopIterations) вне зависимости от того, был ли исходным артефактом BPMN или PlantUML.
...
### 5.4. Компоненты `Execution Engine`

- **`Scheduler` (в `orchestra-api`):**
  - **Роль:** Центральный планировщик.
  - **Функции:** Периодически анализирует `PENDING` задачи, применяет лимиты по тенантам, учитывает приоритеты (UI vs CI) и граф зависимостей `SuiteRun`. Переводит готовые задачи в `QUEUED` и отправляет их в RabbitMQ.
  - **Реализация:** `ADR-0027`.
- **`Executor` (в `orchestra-executor`):**
  - **Роль:** Безсостоящий исполнитель.
  - **Функции:** Берёт "аренду" на `TestRun`, выполняет шаги, обновляет `ExecutionContext` и `heartbeat`, фиксирует статусы и метрики.
  - **Реализация:** `ADR-0020`.
- **`Reaper` (в `orchestra-api` / `orchestra-executor`):**
  - **Роль:** Watchdog.
  - **Функции:** Отслеживает устаревшие `heartbeat_at` или истёкшие `lock_until`, переводит зависшие прогоны в `FAILED_STUCK`, освобождает ресурсы.
  - **Реализация:** `ADR-0020`.
- **`Provisioning Service` (логический, в `orchestra-api`):**
  - **Роль:** Автоматизированный онбординг.
  - **Функции:** Создаёт ресурсы для нового тенанта (записи в БД, конфигурации в Keycloak, очереди/виртуальные хосты в RabbitMQ) и настраивает квоты (`tenant_settings`).
  - **Реализация:** `TASK-2024-102`.

### 5.5. Режимы исполнения и отладка
...
### 7.1. Генерация данных: Паттерн "Planner + Resolver" (Java Implementation)

- **`ai-service` (Planner):**
  - Использует **Spring AI** и модель **`gpt-oss:20b`**.
  - **Prompt Engine** загружает актуальный системный промпт из БД.
  - Агент анализирует запрос и генерирует `DataPlan` (JSON), используя `BeanOutputParser` для гарантии структуры.
- **`Data Resolver` (Tool & RAG):**
  - Реализован как Java-компонент внутри `orchestra-api`.
  - **Vector Search:** Использует **Spring AI `VectorStore`** поверх **PGVector** для выполнения семантического поиска (RAG).
  - **Embeddings:** Векторизация текста выполняется моделью **`mxbai-embed-large`** (через Ollama).
  - **Логика:** Резолвер комбинирует точный поиск (SQL WHERE) и семантический поиск (Cosine Similarity), чтобы найти данные, соответствующие критериям плана (например, «найти клиента, похожего на VIP, но с долгом»).
- **Agent Loop:** `ai-service` самостоятельно управляет циклом: "Сгенерировать план" -> "Проверить валидность" -> "Вернуть результат".

### 7.2. Другие роли `ai-service`

- **Аналитик отчетов:** Java-агент, использующий инструмент чтения JSON-отчетов и генерирующий рекомендации на основе промптов из БД.
- **Mapping Agent:** Использует семантические возможности `gpt-oss:20b` для сопоставления текстов задач BPMN и OpenAPI операций.
...
- **Паттерн `Tenant Context Propagation`:** Для каждого запроса `tenant_id` извлекается из JWT, сохраняется в `TenantContextHolder` (`ThreadLocal`) и на уровне транзакции передаётся в PostgreSQL через `SET LOCAL app.current_tenant = '...'`. Это гарантирует, что RLS-политики и явные фильтры в репозиториях всегда работают с корректным контекстом тенанта (см. `ADR-0028`).
