...
- **ai-service** – **"AI Core" / LLM Gateway**. Центральный сервис для интеллектуальных задач (Planner, Analyst, Assistant), инкапсулирующий взаимодействие с LLM.
...
- **`Data Resolver` и `Agent Toolbox` (в `orchestra-api`):** `Data Resolver` — ключевой инструмент `Agent Toolbox`. Он принимает `DataPlan`, использует кастомные расширения OpenAPI (`x-orchestra-entity`, `x-orchestra-resolver`), строит детерминированные SQL/DSL-запросы и возвращает реальные данные из тестовой БД. `Agent Toolbox` также включает дополнительные инструменты (получение схем API, анализ отчетов, helper'ы для передачи данных), что позволяет AI-агенту выполнять роли Planner/Analyst/Assistant, оставаясь в строгих границах детерминированных операций.
