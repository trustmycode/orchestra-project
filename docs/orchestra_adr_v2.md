...
- Разработчики устанавливают Ollama на локальные машины и скачивают рекомендованную модель (**gpt-oss:20b** или **qwen2.5-coder:14b**).
...
- Установка одной командой + `ollama pull gpt-oss:20b`.
...
Ранее планировалось использовать Python-фреймворк `sgr-agent-core`. В связи с политикой компании (запрет на Python) и требованием унификации стека, данное решение отменено.

### Решение

Изначальный план был заменить внутреннюю реализацию агентного цикла на `sgr-agent-core`, однако решение отменено, чтобы перейти на Java-only стек (см. ADR-0031).
---
...
## ADR-0031: Java Agent Runtime на Spring AI вместо `sgr-agent-core`
...
Изначально `ADR-0025` предполагал использование Python-фреймворка. Политика компании запрещает использование Python, требуя унификации стека вокруг Java и Spring Boot. Нам необходимы возможности структурированного вывода (JSON) и агентного цикла (Planner + Tools).

### Решение

- Реализовать в `ai-service` компонент **Java Agent Runtime** на базе **Spring AI**.
- Использовать модель **`gpt-oss:20b`** (через Ollama) как основную LLM, так как она показывает отличные результаты в генерации структур данных и кода.
- Реализовать собственный цикл агента (Loop): Промпт -> LLM -> Парсинг -> Вызов Java-инструмента (@Tool) -> LLM.
- Использовать `BeanOutputConverter` из Spring AI для строгого маппинга ответов в Java Records/DTO.

### Последствия

**Плюсы:**
- Единый стек: Java 21 + Spring Boot.
- Отсутствие sidecar-контейнеров на Python.
- Полная типизация и интеграция с существующими Java-сервисами.

**Минусы:**
- Необходимость самостоятельной реализации цикла агента (вместо готового во фреймворке).

---

## ADR-0032: Хранение и управление промптами в БД
...
Хардкод промптов в Java-коде или файлах ресурсов требует пересборки и передеплоя приложения для любого изменения логики AI. "Prompt Engineering" требует быстрого цикла итераций.

### Решение

- Хранить шаблоны промптов (System, User) в базе данных PostgreSQL (таблица `ai_prompts`).
- Реализовать `PromptManager` с кэшированием (Caffeine) для высокой производительности.
- Предоставить API для горячего обновления промптов без даунтайма.
- Поддерживать версионирование промптов.

### Последствия

**Плюсы:**
- Гибкость настройки поведения агентов в runtime.
- Возможность A/B тестирования промптов (в будущем).

**Минусы:**
- Дополнительная нагрузка на БД (смягчается кэшем).
- Риск "сломать" агента некорректным обновлением промпта в БД (нужна валидация).

---

## ADR-0033: Реализация RAG с использованием Spring AI VectorStore и PGVector
...
Для реализации паттерна "Planner + Resolver" (ADR-0019) требуется механизм семантического поиска (RAG). В связи с переходом на чистый Java-стек (ADR-0031), использование Python-библиотек для работы с векторными БД недопустимо.

### Решение

1.  **Абстракция:** Использовать интерфейс **`VectorStore`** из фреймворка **Spring AI**. Это позволяет работать с векторной БД в стиле Spring Data, скрывая низкоуровневые SQL-запросы.
2.  **Хранилище:** Использовать **PostgreSQL** с расширением **`pgvector`** (реализация `PgVectorStore` в Spring AI). Это позволяет хранить реляционные данные и векторы в одной БД, упрощая транзакционность и бэкапы.
3.  **Модель Embeddings:** Использовать модель **`mxbai-embed-large`** через Ollama. Это SOTA модель с открытым кодом, оптимизированная для RAG задач, с поддержкой длинного контекста и высокой производительностью.

### Последствия

**Плюсы:**
- Нативная Java-реализация RAG.
- Отсутствие необходимости поднимать отдельную векторную БД (Milvus/Chroma/Weaviate), используется существующий Postgres.
- Легкая замена модели эмбеддингов через конфигурацию Spring AI.

**Минусы:**
- Зависимость от расширения `pgvector` в Postgres (требует специального Docker-образа или установки расширения).
