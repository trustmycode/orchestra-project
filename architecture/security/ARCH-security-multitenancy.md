...
### 2. Изоляция Секретов
*   **`TenantSecret`**: Секреты строго привязаны к `tenant_id`.
*   **Executor Security**: При выполнении теста `orchestra-executor` загружает только те секреты, которые принадлежат тенанту, запустившему `TestRun`. Это предотвращает утечку кредешиалов между клиентами платформы.

### 3. Изоляция ресурсов ("Шумный сосед")
